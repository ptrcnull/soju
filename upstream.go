...
func (uc *upstreamConn) handleMessage(ctx context.Context, msg *irc.Message) error {
	...
	case "QUIT":
		...
		uc.users.Delete(msg.Prefix.Name)
	...
	case rpl_whospcrpl:
		...
		if len(cmd.Params) > 1 {
			fields, _ := parseWHOXOptions(cmd.Params[1])
			if strings.IndexByte(fields, 'n') < 0 {
				return nil
			}
			info, err := parseWHOXReply(msg, fields)
			if err != nil {
				return err
			}
	...
	case "AWAY":
		...
		uu := uc.users.Value(msg.Prefix.Name)
		if uu != nil && uu.Flags != "" {
			flags := uu.Flags
...
func (uc *upstreamConn) getCachedWHO(mask, fields string) (l []*upstreamUser, ok bool) {
	...
	if strings.IndexByte(fields, 'a') >= 0 && !uc.caps["account-notify"] {
		return nil, false
	}
	if strings.IndexByte(fields, 'f') >= 0 && !uc.caps["away-notify"] {
		return nil, false
	}

	if uu := uc.users.Value(mask); uu != nil {
		if uu.hasWHOXFields(fields) {
			return []*upstreamUser{uu}, true
		}
	} else if uch := uc.channels.Value(mask); uch != nil {
		l = make([]*upstreamUser, 0, len(uch.Members.innerMap))
		for nick := range uch.Members.innerMap {
			uu := uc.users.Value(nick)
...
func (uc *upstreamConn) cacheUserInfo(nick string, info *upstreamUser) {
	...
	if uu == nil {
		...
		uc.users.SetValue(nick, info)
	} else {
		uu.updateFrom(info)
		if info.Nickname != "" && nick != info.Nickname {
			uc.users.Delete(nick)
			uc.users.SetValue(info.Nickname, uu)
		}
	}
}
